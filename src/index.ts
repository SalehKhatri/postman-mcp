  ...
  try {
    switch (name) {
      ...
      case TOOL_NAMES.UPDATE_WORKSPACE: {
        const { workspaceId, name, description } =
          schemas.updateWorkspace.parse(args);
        result = await postmanClient.updateWorkspace(
          workspaceId,
          name,
          description
        );
      ...
      case TOOL_NAMES.CREATE_COLLECTION: {
        const { name, description, workspaceId } =
          schemas.createCollection.parse(args);
        result = await postmanClient.createCollection(
          name,
          workspaceId,
          description
        );
      ...
      case TOOL_NAMES.UPDATE_COLLECTION: {
        const { collectionUid, info, variables } =
          schemas.updateCollection.parse(args);

        // First, get the current collection
        const currentCollection = await postmanClient.getCollection(
          collectionUid
        );
        ...
        result = await postmanClient.updateCollection(
          collectionUid,
          currentCollection
        );
        break;
      ...
      case TOOL_NAMES.UPDATE_REQUEST: {
        const { collectionUid, requestName, updates } =
          schemas.updateRequest.parse(args);
        result = await postmanClient.updateRequest(
          collectionUid,
          requestName,
          updates
        );
      ...
      case TOOL_NAMES.DELETE_REQUEST: {
        const { collectionUid, requestName } =
          schemas.deleteRequest.parse(args);
      ...
      case TOOL_NAMES.CREATE_ENVIRONMENT: {
        const { name, values, workspaceId } =
          schemas.createEnvironment.parse(args);
        result = await postmanClient.createEnviromnt(
          name,
          values || [],
          workspaceId
        );
      ...
      case TOOL_NAMES.UPDATE_ENVIRONMENT: {
        const { environmentUid, name, values } =
          schemas.updateEnvironment.parse(args);
        result = await postmanClient.updateEnvironment(
          environmentUid,
          name,
          values
        );
      ...
      case TOOL_NAMES.CREATE_FOLDER: {
        const { collectionUid, folderName, parentFolder } =
          schemas.createFolder.parse(args);
        result = await postmanClient.createFolder(
          collectionUid,
          folderName,
          parentFolder
        );
